...
we can simply render the initial screen and then pass of execution to the client. In this way, we get both the benefits of a an app that is initially
...
of JavaScript code at runtime. Because the user doesn't actually need all that code when the app loads, we can use techniques like bundle splitting
...
As always, it depends. If we're building a simple site, like a blog, we may want to consider using SSG, as we can get all of the benefits with none
